...
def plot_via_points_trajectory(via_points, L1, L2, L3, t_0, t_f_per_segment):
    ...
    # Update these topics to match Gazebo controller topics
    ...
    # After trajectory simulation, return to home position (0, 0, 0)
    rospy.loginfo("Returning to home position (q1=0, q2=0, q3=0).")
    
    joint1_pub.publish(0.0)
    joint2_pub.publish(0.0)
    joint3_pub.publish(0.0)

    # Wait to ensure the robot reaches the home position
    rospy.sleep(2)
...
if __name__ == '__main__':
    try:
        ...
        # Define via points in Cartesian space (X, Y, theta)
       	via_points = [
            (3, 0, np.pi / 4), # Starting Point
	    (1.5, 1.5, np.pi /4),  # Intermediate via point 1
            (0, 3, np.pi / 4)	  # Final Point
        ]
